...
const validateRegisterUserInfo = (req, res, next) => {
  try {
    if (!req.body) throw new Error(VALIDATION_ERRORS.INVALID_PAYLOAD);
    const { error: validationError } = registerUserSchema.validate(req.body);
    if (validationError) {
      throw new Error(
    ...
    console.error('Error in validateRegisterUserInfo - ', error);
    res.status(400).json({
      success: false,
...
module.exports = validateRegisterUserInfo;