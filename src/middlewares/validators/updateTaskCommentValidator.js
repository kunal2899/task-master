...
const { updateTaskCommentSchema } = require("../../schemas/taskCommentsSchema");
...
const validateUpdateTaskComment = (req, res, next) => {
  try {
    ...
    const { error: validationError } = updateTaskCommentSchema.validate({
      teamId,
      userId,
      taskId,
      commentId,
    ...
    console.error("Error in validateUpdateTaskComment - ", error);
    res.status(400).json({
      success: false,
...
module.exports = validateUpdateTaskComment;
